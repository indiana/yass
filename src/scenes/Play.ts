...
export class Play extends Phaser.Scene {
    ...
    create() {
        ...
        if (!this.anims.exists('player_flame')) {
            this.anims.create({
                key: 'player_flame',
        ...
        // Ensure enemies are immovable
        this.enemyPool.getChildren().forEach((enemy: any) => {
    ...
    update(time: number) {
        ...
        this.playerBulletPool.getChildren().forEach((bullet: any) => {
            if (bullet.active) bullet.update();
        });
        this.enemyBulletPool.getChildren().forEach((bullet: any) => {
            if (bullet.active) bullet.update();
        });
        this.enemyPool.getChildren().forEach((enemy: any) => {
            if (enemy.active) enemy.update();
        });
        this.powerupPool.getChildren().forEach((powerup: any) => {
